...
class TestSuffixVersionMajorMinorPatchPrerelease(TestRoutineCommitMessage):
    def setUp(self):
        super().setUp()
        self.routine = self.get_routine('SuffixVersionMajorMinorPatchPrerelease')

    @parameterized.expand([
        (SemverTag.init_spec(1, 0, 0), 'Initial Commit', 'Initial Commit [`1.0.0`]'),
        (SemverTag.init_spec(2, 4, 6, 'rc.3', 'build.4'), 'F: Calculate Zombie Damage', 'F: Calculate Zombie Damage [`2.4.6-rc.3`]'),
        (SemverTag.init_spec(0, 0, 0, build='build.1'), 'A: Heyy!', 'A: Heyy! [`0.0.0`]'),
        (SemverTag.init_spec(0, 0, 0, prerelease='rc.1'), ':3', ':3 [`0.0.0-rc.1`]'),
        (SemverTag.init_spec(0, 0, 0, prerelease='rc.3', build='build.7'), ':3', ':3 [`0.0.0-rc.3`]'),
    ])
